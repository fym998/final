...
def get_whois_info(domain):
    ...
    apis = [
        # WhoisXMLAPI (需要API密钥)
        lambda d: f"https://www.whoisxmlapi.com/whoisserver/WhoisService?apiKey={WHOIS_API_KEY}&domainName={d}&outputFormat=JSON"
        if WHOIS_API_KEY
        else None,
        # Whois API (免费但有限制)
                ...
                if "json" in api_url or "JSON" in api_url:
                    try:
                        ...
                        org = (
                            ...
                            .get("registrant", {})
                            .get("organization")
                            or data.get("registrant", {}).get("organization")
                            or data.get("org")
...
def process_buckets(bucket_list):
    """处理密钥桶列表"""
    processed_buckets = []

    for bucket_item in tqdm(bucket_list, desc="处理密钥桶"):
        bucket_info = bucket_item.get("bucket_info", "")
        source = bucket_item.get("source", "")

        # 检查是否与车企相关
        if is_auto_related(bucket_info):
            processed_buckets.append(
                {"bucket_info": bucket_info, "source_file": source}
            )
...
def process_certificates(cert_list):
    ...
    for cert_item in tqdm(cert_list, desc="处理证书"):
        cert_content = cert_item.get("content", "")
        source = cert_item.get("source", "")

        # 这里可以添加证书解析逻辑
        # 目前只保存原始内容
        processed_certs.append({"cert_content": cert_content, "source_file": source})

    return processed_certs


def process(input_data: dict, output_path: str = "step3_output.json"):
    # 处理IP地址
    ...
    processed_certificates = process_certificates(certificates)

    # 输出统计信息
    print(f"发现公网IP地址数量: {len(processed_ips)}")
    print(f"发现车企相关域名数量: {len(processed_domains)}")
    print(f"发现车企相关密钥桶数量: {len(processed_buckets)}")
    print(f"发现证书数量: {len(processed_certificates)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="数据分析工具 - 从step2_output.json读取数据并分析"
    )
    parser.add_argument(
        "-i", "--input", default="step2_output.json", help="输入JSON文件路径"
    )
    parser.add_argument(
        "-o", "--output", default="step3_output.json", help="输出JSON文件路径"
    )
